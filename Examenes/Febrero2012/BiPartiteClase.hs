...
aux g dict stack
   ...
   | not (colored v) = aux g (D.insert v c dict) (pushAll stack' sucesores) 
   | colored v = if c /= fromJust (D.valueOf v dict) then Nothing else aux g dict stack'
--
...
--
  where
    (v,c) = S.top stack
    stack' = S.pop stack 
    sucesores = [ (v',nextColor c) | v' <- successors g v, not (colored v') ]
