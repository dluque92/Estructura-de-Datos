...
floor :: Ord a => BST a -> a -> Maybe a
floor Empty x    = Nothing
floor (Node y w lt rt) x = if k == y then Just y else
  if x < y then floorL else
    Just $ case floorR of
      Nothing -> y
      Just rtx -> rtx
...
ceiling :: Ord a => BST a -> a -> Maybe a
ceiling Empty _ = Nothing
ceiling (Node y w lt rt) x = if x == y then Just x else
  if x > y then ceiling x rt else
     Just $ case ceiling x lt of
          Nothing -> y
          Just ltx -> ltx
